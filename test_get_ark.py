...
def _get_ark(self, input_token = 'BLANK', file_type='csv', merge=True):
    ...
    for x in range(row_count):
        ...
        reference.append((x,descIDS_list,fsid,uid))
        if len(descIDs) >= 30:
            descIDs_cache.append()
        #######################################################################
        # there is no check here so the looping will take longer every time as
        # the reference is never rest. sorry but I don't have time to fix this
        # right now so I hope someone else will shortly.
        # i imagine there should be a check for the length of the object reference
        # and then a resetting of that object to 0 elements
        #######################################################################
        
        #Once the list is long enough query family search about our descIDs
        for ID in reference[x][1]:
